...
public class ConsumerPartitionAssignSeek {
    ...
    public static void main(String[] args) {
        ...
        Runtime.getRuntime().addShutdownHook(new Thread() {
            public void run() {
                ...
                try {
                    mainThread.join(); //Main Thread가 poll()을 빠져나오고 close()를 수행한 뒤 정상적으로 종료 될 때가지 기다렸다고 종료되도록 Main Thread.join()을 수행
    ...
    public static void pollNoCommit(KafkaConsumer<String, String> kafkaConsumer) {
        int loopCnt = 0;
        try {
            while (true) {
    ...
    public static void pollCommitAsync(KafkaConsumer<String, String> kafkaConsumer) {
        int loopCnt = 0;
        try {
            while (true) {
                ConsumerRecords<String, String> consumerRecords = kafkaConsumer.poll(Duration.ofMillis(1000));
